...
| `npm run test:e2e` | Runs the daily matrix: 34 Chromium functional cases, 20 browser-critical cases in Firefox/WebKit, and 9 visual cases. |
| `npm run test:e2e:functional` | Runs the 34 structural and interaction cases in Chromium without visual snapshots. |
| `npm run test:e2e:browser-critical` | Runs the 10 critical scenarios in Firefox and WebKit (20 executions). |
| `npm run test:e2e:smoke` | Runs the 2 public smoke scenarios in Chromium, Firefox, and WebKit (6 executions). |
| `npm run test:e2e:cross-browser` | Runs all 34 functional scenarios in Chromium, Firefox, and WebKit (102 executions). |
| `npm run test:e2e:full` | Runs the complete 111-execution matrix, including visual coverage. |
...
Playwright starts an isolated production server on `127.0.0.1:3218`. Chromium runs all 34 functional scenarios and the approved visual coverage. Firefox and WebKit run the 10 scenarios tagged `@cross-browser` in the daily matrix; all 34 functional scenarios remain available in both engines through `test:e2e:cross-browser` and `test:e2e:full`. Visual snapshots remain restricted to Chromium on Windows.
...
GitHub Actions runs lint and build, the 34 Chromium functional cases, the 20 Firefox/WebKit browser-critical executions, and the 9 Windows visual cases for pull requests into `integration/**`, `dev`, or `main`. Manual workflow runs execute all 102 functional cross-browser cases and the 9 visual cases. Always verify check results against the pull request's current head commit. Reports and failure artifacts are retained for seven days.
