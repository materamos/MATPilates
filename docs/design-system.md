...
- The studio gallery advances automatically every five seconds when it has multiple images and is not paused. Motion animates its transform for 400 ms, preserves the established leftward automatic direction, and performs the duplicate-first-slide reset without a visible reverse transition. Horizontal drag supports next and previous navigation while `touch-action: pan-y` preserves page scrolling. Clicking the gallery or pressing Enter or Space toggles pause and resume; a completed drag does not toggle that state.
