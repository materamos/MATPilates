...
Classes, schedule, and reservation use a 32 px vertical content inset in Mobile and Tablet, and a 60 px inset in Compact Narrow, Compact Content, and Desktop. The weekly schedule uses exclusive day accordions below 1024 px and a semantic day-by-time table at 1024 px and above. Both views render the same structured source and link every scheduled offering to its class-catalog disclosure. In wide compositions, the reservation copy and image stretch between those limits so the content envelope preserves the same top and bottom spacing. Manifesto and footer retain their own component-specific vertical rhythm.
...
Full-page snapshots cover the representative 320, 390, 768, 1077, 1280 x 720, 1280 x 901, and 1440 compositions. Structural tests cover the remaining boundaries, navigation mode, map eligibility, intrinsic section sizing, horizontal overflow, section insets, and the schedule's 6-day/52-slot data contract. Interaction tests cover menu focus transfer, exclusive class and schedule disclosures, schedule-to-catalog navigation, reduced-motion gallery control, and the documented focus outline.
